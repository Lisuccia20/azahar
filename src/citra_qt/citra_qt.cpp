...
GMainWindow::GMainWindow(Core::System& system_)
    ...
    if (auto* streamer = system.GetScreenStreamer()) {
        std::cerr << "[Theme] ScreenStreamer trovato, collego callback tema\n";
        streamer->SetOnConnectedCallback([this]() {
            QMetaObject::invokeMethod(this, "OnStreamerConnected", Qt::QueuedConnection);
            std::cerr << "[DEBUG] SetOnConnectedCallback chiamato\n";
        });
...
void GMainWindow::BootGame(const QString& filename) {
    ...
    if (auto remote = InputCommon::GetRemoteSwitch()) {
        remote->SetNavigationCallback(nullptr);
        remote->SetStickCallback(nullptr);
    }
...
void GMainWindow::UpdateUITheme() {
    ...
    // ── Se lo streamer è connesso, forza il tema Switch OLED ─────────────────
    const QString& current_theme = streamer_connected
        ? QStringLiteral("switch_oled")
...
void GMainWindow::OnStreamerConnected() {
...
void GMainWindow::OnStreamerDisconnected() {
...
// Mapping button ID Switch → navigazione grid
// ID standard Switch HID: A=0, B=1, X=2, Y=3,
...
void GMainWindow::OnRemoteSwitchButton(int button_id) {
    ...
    case 0:  // A → avvia
        ...
        break;
    case 18: // D-pad Sinistra
        game_list->GridNavigate(GameList::GridDirection::Left);
        break;
    case 19: // D-pad Destra
        game_list->GridNavigate(GameList::GridDirection::Right);
        break;
    default:
        break;
    }
...
void GMainWindow::OnRemoteSwitchStick(float x, float y) {
    if (game_list->CurrentViewMode() != GameList::ViewMode::Grid)
        return;
    ...
    // Debounce: accetta un input ogni 200ms
    const auto now = std::chrono::steady_clock::now();
    const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - last_stick_nav_time_).count();
    if (elapsed < 300)
        return;
    last_stick_nav_time_ = now;

    if (std::abs(x) >= std::abs(y))
        game_list->GridNavigate(x > 0 ? GameList::GridDirection::Right
                                       : GameList::GridDirection::Left);
    else
        game_list->GridNavigate(y > 0 ? GameList::GridDirection::Down
                                       : GameList::GridDirection::Up);
}
